...
describe('Hand', () => {
  describe('rendering', () => {
    it('renders the flatlist for the hand', () => {
      const testRenderer = renderer.create(<Hand 
        cards={[]} boardCards={[]} canPlace={true} placeCard={jest.fn()} placeSpell={jest.fn()} 
        enableTargetMode={jest.fn()} targetMode={1} actionPoints={10} gameState={gameState} />);
    ...
    it('renders the card component', () => {
        const testRenderer = renderer.create(<Hand 
            cards={cards} boardCards={[]} canPlace={true} placeCard={jest.fn()} placeSpell={jest.fn()} 
            enableTargetMode={jest.fn()} targetMode={1} actionPoints={10} gameState={gameState} />);
          const testInstance = testRenderer.root;
