  ...
  describe('rendering', () => {
    ...
    it('renders the playeravatars', () => {
        const testRenderer = renderer.create(
          <Gameboard playerName="test" playerActionPoints={10} playerCards={[]} opponent="test"
          playerHealthPoints={30} opponentCards={[]} opponentActionPoints={10} opponentHealthPoints={30}
          attackCard={jest.fn()}  attackPlayer={jest.fn()} activityAttackTarget={jest.fn()}
          turn="start" phase="start" setTarget={jest.fn()} targetCard={card}
          gameState={gameState} activityCurrentAttackerUniquePlayId={0} activityCurrentTargetUniquePlayId={1} />
        );
    ...
    it('renders the fields of both players', () => {
      ...
      it('renders the turn info', () => {
        ...
        const testInstance = testRenderer.root;
        expect(testInstance.findAllByProps({testID: 'turnInfoText'})).not.toBeNull();
      });
  });
});
