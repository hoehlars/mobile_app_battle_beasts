...
describe('PaymentService', () => {
  ...
  it('should initialize a payment', async () => {
    ...
    const redirectUrl: string = await (await PaymentService.initializePayment('mytoken', 2)).json();
    expect(redirectUrl).toStrictEqual('www.sandbox.paypal.com');
    expect(fetchMock).toHaveBeenCalledWith('http://localhost:5000/api/cards/buy', {
      body: '{"amount":2}',
  ...
  it('should execute a payment and get new cards if payment was approved by paypal', async () => {
    ...
    const card = await (await PaymentService.executePaymentAndGetBoughtCards('mytoken', 2, payerId, paymentId)).json();

    expect(card).toEqual([{ cardId: 1, duplicate: false }]);
    expect(fetchMock).toHaveBeenCalledWith('http://localhost:5000/api/cards/buy/execute', {
