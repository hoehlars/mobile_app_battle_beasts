...
describe('DeckManagerScreen', () => {
  ...
  describe('rendering', () => {
    it('renders the header', () => {
      const testRenderer = renderer.create(<DeckManagerScreen navigation={navigation}/>);
      const testInstance = testRenderer.root;
      expect(testInstance.findByType(Header)).not.toBeNull();
    });
  
    it('renders line below header', () => {
      const {getByTestId} = render(<DeckManagerScreen navigation={navigation} />);
  ...
  describe('logic', () => {
    it('renders the text input when floating action button is pressed', async () => {
      ...
      const testInstance = testRenderer.root;
      expect(testInstance.findAllByType(TextInput).length).toBe(0);
      await act(async () => testInstance.findByType(FloatingAction).props.onPressMain());
    ...
    it('can add an item', async() => {
      ...
      expect(testInstance.findByType(TextInput)).not.toBeNull();
      await act(async () => textInput.props.onSubmitEditing({nativeEvent: {
        text: 'hello'
      }}));
    ...
    it('can delete an item', async () => {
      ...
      // delete it 
      const deleteButton = testInstance.findByProps({testID: 'deleteDeckButton'});
