...
import { NavigationRoute, NavigationScreenProp, NavigationState } from 'react-navigation';
...
class DeckManagerUpdateDeckScreen extends React.Component<DeckManagerUpdateDeckScreenProps, DeckManagerUpdateDeckScreenState> {
  ...
  private async deleteCardOutOfDeck(pressedCard: ListRenderItemInfo<CardFlatListData>): Promise<void> {
    ...
    const patchDeckRes = await DeckService.patchDeck(this.state.token, this.state.deck.name, cards);
  ...
  private showCardInTheMiddle(pressedCard: ListRenderItemInfo<CardFlatListData>): void {
    console.log(pressedCard);
  }

  private async addCardToDeck(pressedCard: ListRenderItemInfo<CardFlatListData>): Promise<void> {
  ...
  private getHighestIdOfOwnedCards(): string {
      if (this.state.cardsInDeck.length >= 1) {
        ...
        const cardHighestId: CardFlatListData = this.state.cardsInDeck.reduce((item1: CardFlatListData, item2: CardFlatListData) => {
  ...
  private renderCardsInDeck(data: ListRenderItemInfo<CardFlatListData>): JSX.Element {
    return(
    <TouchableOpacity 
    onPress={pressed => this.deleteCardOutOfDeck(data)} 
    style={styles.item}
    onLongPress={pressed => this.showCardInTheMiddle(data)}>
       <Text style={styles.title}>{data.item.name}</Text>
    </TouchableOpacity>
  );
    }

    private renderAvailableCards(data: ListRenderItemInfo<CardFlatListData>): JSX.Element {
      return(
      <TouchableOpacity 
      onPress={pressed => this.addCardToDeck(data)} 
      style={styles.item}
      onLongPress={pressed => this.showCardInTheMiddle(data)}>
         <Text style={styles.title}>{data.item.name}</Text>
      </TouchableOpacity>
  ...
  render() {
    return (
      <>
        ...
        {this.state.error === '' ? null : <View style={styles.error}>
          <Text style={styles.TextError}>{this.state.error}</Text>
        </View>}

        {this.state.updatedDeckSuccessfully ? <View style={styles.UpdateSuccess}>
